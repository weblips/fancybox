...
class Brand extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    protected function _construct(){
    	$this->_init('Wise_Fancy','brand_id');
    }
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['Wise_Fancy_store' => $this->getTable('Wise_Fancy_store')],
                $this->getMainTable() . '.brand_id = Wise_Fancy_store.brand_id',
                []
                )->where(
                'status = ?',
                1
                )->where(
                'Wise_Fancy_store.store_id IN (?)',
                $storeIds
                )->order(
                'Wise_Fancy_store.store_id DESC'
                )->limit(
                1
                );
            }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('Wise_Fancy_store')],
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('Wise_Fancy_store'), $condition);

        $condition = ['brand_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('Wise_Fancy_product'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('Wise_Fancy_store');
        ...
        // Posts Related
        $table = $this->getTable('Wise_Fancy_product');
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ($id = $object->getId()) {
                ...
                ->from($this->getTable('Wise_Fancy_product'))
                ->where(
                    'brand_id = '.(int)$id
                    );
    ...
    public function lookupStoreIds($brandId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('Wise_Fancy_store'),
            'store_id'
            )
        ->where(
            'brand_id = ?',
